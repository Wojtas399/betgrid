...
void main() {
  ...
  group('initialize, ', () {
    ...
    tearDown(() {
      verify(
        () => createPlayersPodiumStats.call(season: currentSeason),
      ).called(1);
      verify(
    ...
    blocTest(
      ...
      build: () => createCubit(),
      setUp: () {
        createPlayersPodiumStats.mock(playersPodium: playersPodium);
        createPointsHistoryStats.mock(pointsHistory: pointsHistory);
        getDetailsOfAllDriversFromSeasonUseCase.mock(
          expectedDetailsOfAllDriversFromSeason: allDrivers,
        );
        createBestPoints.mock(expectedBestPoints: bestPoints);
      },
  ...
  group('onStatsTypeChanged, ', () {
    ...
    final List<PointsForDriver> pointsForDrivers = [
      PointsForDriver(driverDetails: allDrivers.first, points: 11),
      PointsForDriver(driverDetails: allDrivers[1], points: 22),
      PointsForDriver(driverDetails: allDrivers.last, points: 33),
    ];
    StatsState? state;
  ...
  group('onDriverChanged, ', () {
    ...
    blocTest(
      ...
      build: () => createCubit(),
      seed: () => const StatsState(stats: IndividualStats()),
    ...
    blocTest(
      ...
      setUp: () {
        seasonCubit.mockState(expectedState: currentSeason);
        createPointsForDriverStats.mock(playersPoints: playersPointsForDriver);
      },
      seed:
          () =>
              state = const StatsState(
                stats: GroupedStats(playersPointsForDriver: []),
              ),
      act: (cubit) => cubit.onDriverChanged('d1'),
      expect:
          () => [
